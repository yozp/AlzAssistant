...
public class AiChatFacade {
     ...
     * @return 保存的目录
     */
    public File generateAndSaveFacade(String userMessage, ChatTypeEnum chatTypeEnum) {
        if (chatTypeEnum == null) {
            throw new BusinessException(ErrorCode.SYSTEM_ERROR, "对话类型为空");
        }
        return switch (chatTypeEnum) {
            case CHAT_TYPE_ENUM -> generateAndSaveChat(userMessage);
            default -> {
     ...
     * @return 保存的目录
     */
    private File generateAndSaveChat(String userMessage) {
        String result = aiChatService.chatToAi(userMessage);
     ...
     * @return 保存的目录
     */
    public Flux<String> generateAndSaveStreamFacade(String userMessage, ChatTypeEnum chatTypeEnum) {
        if (chatTypeEnum == null) {
            throw new BusinessException(ErrorCode.SYSTEM_ERROR, "对话类型为空");
        }
        return switch (chatTypeEnum) {
            case CHAT_TYPE_ENUM -> generateAndSaveChatStream(userMessage);
            default -> {
                String errorMessage = "不支持的对话类型：" + chatTypeEnum.getValue();
                throw new BusinessException(ErrorCode.SYSTEM_ERROR, errorMessage);
            }
     ...
     * @return 保存的目录
     */
    private Flux<String> generateAndSaveChatStream(String userMessage) {
        Flux<String> result = aiChatService.chatToAiStream(userMessage);
        // 当流式返回生成代码完成后，再保存代码
