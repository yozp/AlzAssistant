...
import com.yzj.alzassistant.mapper.ChatHistoryMapper;
...
public class ChatHistoryServiceImpl extends ServiceImpl<ChatHistoryMapper, ChatHistory>  implements ChatHistoryService{
    ...
    @Override
    public boolean addChatMessage(Long appId, String message, String messageType, Long userId, String attachmentsJson) {
        ...
        if (ChatHistoryMessageTypeEnum.USER.equals(messageTypeEnum)) {
            ThrowUtils.throwIf(StrUtil.isBlank(message) && StrUtil.isBlank(attachmentsJson),
                    ErrorCode.PARAMS_ERROR, "消息内容与附件不能同时为空");
        } else {
            ThrowUtils.throwIf(StrUtil.isBlank(message), ErrorCode.PARAMS_ERROR, "消息内容不能为空");
        }
        ChatHistory chatHistory = ChatHistory.builder()
                .appId(appId)
                .message(StrUtil.nullToDefault(message, ""))
                .attachments(StrUtil.isBlank(attachmentsJson) ? null : attachmentsJson.trim())
        ...
        try {
            ...
            for (ChatHistory history : historyList) {
                if (ChatHistoryMessageTypeEnum.USER.getValue().equals(history.getMessageType())) {
                    String userText = history.getMessage();
                    if (StrUtil.isBlank(userText) && StrUtil.isNotBlank(history.getAttachments())) {
                        userText = "（附件）";
                    }
